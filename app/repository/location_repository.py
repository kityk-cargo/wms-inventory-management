...
def get_by_identifiers(db: Session, aisle: str, bin: str) -> Location:
    return db.query(Location).filter(Location.aisle == aisle, Location.bin == bin).first()
...
def update_location(db: Session, existing_location: Location, update_data: dict) -> Location:
...
def exists(db: Session, aisle: str, bin: str) -> bool:
    ...
    return bool(db.query(Location).filter(Location.aisle == aisle, Location.bin == bin).first())
